...
export default function ChartOfAccount() {
  ...
  const handleEdit = (item) => {
    navigate(`/editchartofaccount/${item.id}`);
  };
  ...
  const confirmDelete = async () => {
    if (accountToDelete && accountToDelete.id !== undefined) {
      try {
        await deleteItem(accountToDelete.id);
        setShowModal(false);
      ...
      <div className="d-flex flex-row justify-content-start vw-100">
        ...
        >
          ...
          <div style={{ marginTop: 50, padding: "2rem" }}>
            <div className="alert alert-info">Loading chart of accounts...</div>
          </div>
        </div>
      </div>
    );
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p>
                  ...
                  <strong>{accountToDelete?.code}</strong>?
                </p>
              </div>
