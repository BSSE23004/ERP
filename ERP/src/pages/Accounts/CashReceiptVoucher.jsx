...
export default function CashReceiptVoucher() {
  ...
  const vouchers =
    allVouchers?.filter((v) => v.voucher_type === "CASH_RECEIPT") || [];
  ...
  const handleAdd = () => {
    navigate("/addcashreceiptvoucher");
  };

  const handleEdit = (item) => {
    navigate(`/editcashreceiptvoucher/${item.id}`);
  };
        ...
        >
          ...
          <div style={{ marginTop: 50, padding: "2rem" }}>
            <div className="alert alert-danger">
              Error loading cash receipt vouchers
            </div>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div style={{ marginTop: 50, padding: "2rem" }}>
          <DataHeader
            ...
            subtitle="Manage Your Cash Receipt Vouchers"
            onAdd={handleAdd}
            buttonText="Add New Cash Receipt Voucher"
          />
          <DataControls
            onAdd={handleAdd}
            searchValue={search}
