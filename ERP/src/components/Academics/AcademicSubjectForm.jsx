...
export default function AcademicSubjectForm({ id }) {
  const navigate = useNavigate();
  const { id: routeId } = useParams();
  const formId = id || routeId;

  // Fetch all subjects to get editing item if needed
  const { data: subjects, loading: loadingSubjects } = useAPI(
    "/api/academics/subjects/",
  );
  const editingItem = formId
    ? subjects.find((item) => item.id == formId)
    : null;
  ...
  if (loadingSubjects && formId) {
    return (
      <>
        <h3 className="mb-4">Loading...</h3>
        <div className="alert alert-info">Fetching subject details...</div>
      </>
    );
  }
  ...
  return (
    <>
      <h3 className="mb-4">
        {editingItem ? "Edit Academic Subject" : "Create New Academic Subject"}
      </h3>
